...
class Blockchain:
    ...
    def new_transaction(self, datas: dict,
                        tx_type: int):  ##############################################################
        """
...
# 트랜잭션 확인 페이지 - 다 만들었는데 정보 출력이 안된다....ㅠㅠ 나중에 수정해보자!! 정 안되면 그냥 raw 데이터 출력
...
def mypage_correct():
    data = [request.form['correct_target'], request.form['correct_data'],
            cookie_status_check()]  # data = [수정항목, 수정내용, 사용자명]
            ...
            try:
                ...
                if t['type'] == '2':
                    if (t['register'] == svRegister) and (t['surveyTitle'] == svTitle) and (t['maxDate'] == svDate):
                        ...
                        # print("LOOKUP SURVEY (PUB) >>> ", sv)
                        perReward = float(sv['total_reward']) / float(sv['max_people'])
                        return render_template('participating_survey_pub.html', sv=sv, perReward=perReward,
                                               svType=svType)
                else:
                    pass
            except IndexError as err:  # 빈 트랜잭션의 경우 INDEX_ERROR 발생
...
def participating():
    ...
    result = dbMgr.transfer_balance('0xadminaccount', dbMgr.idcheck(cookie_status_check())[0]['hash_key'],
                                    float(request.form['reward']))
    if result != 1:
        return "<script>alert('답변 등록에 실패하였습니다!'); history.back();</script>"
    ...
    if svType == "PRI":
        ...
        for ps in pri_svList:
            if (ps['register'] == svRegister) and (ps['survey_title'] == svTitle) and (ps['max_date'] == svDate):
                ...
                answer3 = []
                for al in ansList:
                    if (al['register'] == svRegister) and (al['survey_title'] == svTitle) and (
                            al['max_date'] == svDate):
